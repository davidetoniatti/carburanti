...
export class Sheet {
  ...
  getTargetY() {
    if (
      ...
      this.panel.classList.contains("mobile-hidden")
    ) {
      return this.type === "bottom" ? window.innerHeight : -window.innerHeight;
    }
    if (this.panel.classList.contains("full")) return 0;
    if (this.panel.classList.contains("peek"))
      return window.innerHeight * (SHEET_CONFIG.PEEK_HEIGHT_VH / 100);
  ...
  onStart(e) {
    ...
    const isHandle =
      e.target.closest(".sheet-handle") || e.target.closest(".sheet-header");
    if (isHandle) {
  ...
  handleBottomEnd(deltaY, velocity) {
    ...
    if (currentTarget === 0) {
      ...
      if (
        deltaY < -threshold ||
        (deltaY < 0 && velocity > SHEET_CONFIG.VELOCITY_THRESHOLD)
      ) {
        this.snapTo("full");
      } else if (
        deltaY > threshold ||
  ...
  handleTopEnd(deltaY, velocity) {
    if (
      deltaY < -SHEET_CONFIG.DRAG_THRESHOLD ||
      (deltaY < 0 && velocity > SHEET_CONFIG.VELOCITY_THRESHOLD)
    ) {
